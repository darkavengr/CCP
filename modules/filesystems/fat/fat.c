...
while(1) {
	...
	while(buf->findentry < ((bpb->sectorsperblock*bpb->sectorsize)/FAT_ENTRY_SIZE)) {
		...
		if((uint8_t) *blockptr == 0xE5) {				/* ignore deleted files */
	...
	if(buf->findentry >= ((bpb->sectorsperblock*bpb->sectorsize)/FAT_ENTRY_SIZE)) {		/* at end of block */
		...
		if((fattype == 12 && buf->findlastblock >= 0xff8) || (fattype == 16 && buf->findlastblock >= 0xfff8) || (fattype == 32 && findblock >= 0x0ffffff8)) {
			kernelfree(blockbuf);
			kernelfree(splitbuf);
...
while(1) {
	...
	while(entry_count < ((bpb->sectorsperblock*bpb->sectorsize)/FAT_ENTRY_SIZE)) {
		...
		if(((unsigned char) *blockptr == 0xE5) || ((unsigned char) *blockptr == 0)) {		/* found free entry */	
			returnvalue=fat_create_entry(type,splitbuf.drive,(uint64_t) readblock,entry_count,fattype,splitbuf.filename,blockbuf,bpb);
	...
	if(entry_count >= ((bpb->sectorsperblock*bpb->sectorsize)/FAT_ENTRY_SIZE)) {		/* at end of block */
		...
		if((fattype == 12 && rb >= 0xff8) || (fattype == 16 && rb >= 0xfff8) || (fattype == 32 && rb >= 0x0ffffff8)) {
			...
			if(fat_update_fat(splitbuf.drive,previousblock,(rb >> 8),(rb & 0xffff)) == -1) {	/* update FAT */
				kernelfree(blockbuf);
				return(-1);
			}

			returnvalue=fat_create_entry(type,splitbuf.drive,rb,(uint64_t) 0,fattype,splitbuf.filename,blockbuf,bpb);
...
if((type == _FILE) || (type == _DIR)) {				/* create file or directory */
	...
	date=((timedate.year-1980) << 11) | (timedate.month << 5) | timedate.day;
	...
	{
		blocksused++;		/* increment number of blocks used */

		startblock=fat_find_free_block(drive);		/* find free block */
		if(startblock == -1) return(-1);

		if(fattype == 12) {
			if(fat_update_fat(drive,startblock,0,0xff8) == -1) return(-1);
		}
		else if(fattype == 16) {
			if(fat_update_fat(drive,startblock,0,0xfff8) == -1) return(-1);
		}
		else if(fattype == 32) {
			if(fat_update_fat(drive,startblock,0x0fff,0xfff8) == -1) return(-1);
		}
	}
	...
	if(blockio(DEVICE_WRITE,drive,(uint64_t) rb,blockbuf) == -1) return(-1); 	/* update directory block for entry */
}
...
if(type == _DIR) {
	if(fat_create_subdirectory_entries(splitbuf.filename,rb,startblock,bpb,time,date,fattype,drive) == -1) return(-1);

	blocksused++;		/* increment number of blocks used */
}

if(fattype == 32) {
	if(fat_update_free_block_count(drive,-blocksused) == -1) return(-1);		/* update free block count */
}
...
blockbuf=kernelalloc(MAX_BLOCK_SIZE);			/* allocate block buffer */
if(blockbuf == NULL) {
	setlasterror(NO_MEM);
	return(-1);
}
			
if(blockio(DEVICE_READ,splitbuf.drive,(uint64_t) buf.findlastblock,blockbuf) == -1) {	/* read block */
	kernelfree(blockbuf);
	return(-1);
}
...
if(blockio(DEVICE_WRITE,splitbuf.drive,(uint64_t)buf.findlastblock,blockbuf) == -1) {	 /* write block */
	kernelfree(blockbuf);
	return(-1);
}
...
/* remove FAT entries */
unlink_block=fat_get_start_block(splitbuf.drive,filename);		/* get file's start block */

do {
	/* remove FAT entry */
	previous_block=unlink_block;

	if(fat_update_fat(splitbuf.drive,(uint64_t) unlink_block,(uint16_t) 0,(uint16_t) 0) == -1) return(-1);

	unlink_block=fat_get_next_block(splitbuf.drive,previous_block);	/* get next block */
	
} while((fattype == 12 && unlink_block <= 0xff8) || (fattype == 16 && unlink_block <= 0xfff8) || (fattype == 32 && unlink_block <= 0x0ffffff8));
...
if((read_file_record.flags & FILE_POS_MOVED_BY_SEEK) && (read_file_record.currentblock != read_file_record.previousblock) \
   && ((read_file_record.currentblock != (read_file_record.previousblock+1)))) {
	...
	if(read_file_record.currentpos < read_file_record.previouspos) read_file_record.currentblock=read_file_record.startblock;
	...
	count=read_file_record.currentpos / (bpb->sectorsperblock*bpb->sectorsize);

	while(count-- !=	 0) {
		read_file_record.currentblock=fat_get_next_block(read_file_record.drive,read_file_record.currentblock);
		if(read_file_record.currentblock == -1) return(-1);
	}
}
...
if(blockio(DEVICE_READ,read_file_record.drive,(uint64_t) blockno,iobuf) == -1) {		/* read block */
	kernelfree(iobuf);
	return(-1);
}

while(read_size > 0) {
//	kprintf_direct("read_size=%X\n",read_size);
	...
	if(read_size >= (bpb->sectorsperblock*bpb->sectorsize)) {
		count=(bpb->sectorsperblock*bpb->sectorsize)-blockoffset;	/* adjust size */

	//	kprintf_direct("EVIL count=%X\n",count);

		/* the remainder of the data will be read on the next cycle */
	}
	else
	{
		count=read_size;

	//	kprintf_direct("NICE count=%X\n",count);
	}

	//kprintf_direct("filename=%s\n",read_file_record.filename);
	//kprintf_direct("addr=%X\n",addr);
	//kprintf_direct("iobuf=%X\n",iobuf);
	//kprintf_direct("blockoffset=%X\n",blockoffset);
	//kprintf_direct("iobuf+blockoffset=%X\n",iobuf+blockoffset);
	//kprintf_direct("count=%X\n",count);
	//kprintf_direct("block size=%X\n",bpb->sectorsperblock*bpb->sectorsize);
	//kprintf_direct("read_file_record.currentpos=%X\n",read_file_record.currentpos);
	//asm("xchg %bx,%bx");

	memcpy(addr,iobuf+blockoffset,count);			/* copy from to buffer */
	...
	/* if reading data that crosses a block boundary, find next block to read data */

	if((read_file_record.currentpos+count+blockoffset) >= round_up(read_file_record.currentpos,(bpb->sectorsperblock*bpb->sectorsize))) {
	 	read_file_record.currentblock=fat_get_next_block(read_file_record.drive,read_file_record.currentblock);
		blockoffset=0;
...
if((write_file_record.currentpos >= write_file_record.filesize) || \
   ...
   (fattype == 32 && write_file_record.currentblock >= 0xffffff8)) {
	...
	if((write_file_record.startblock == 0) || (write_file_record.currentpos % (bpb->sectorsperblock*bpb->sectorsize) == 0)) {
		...
		do {
			if(fat_update_fat(write_file_record.drive,lastblock,(newblock >> 16),(newblock & 0xffff)) == -1) {
				kernelfree(iobuf);
				return(-1);
			}
			...
			if(fattype == 12) {
				if(fat_update_fat(write_file_record.drive,newblock,0,0xff8) == -1) {
					kernelfree(iobuf);
					return(-1);
				}
				...
				if(fat_update_fat(write_file_record.drive,newblock,0,0xfff8)  == -1) {
					kernelfree(iobuf);
					return(-1);
				}
			}
			else if(fattype == 32) {
				if(fat_update_fat(write_file_record.drive,newblock,0x0fff,0xfff8) == -1) {
					kernelfree(iobuf);
					return(-1);
				}
			}
...
if(blockio(DEVICE_READ,write_file_record.drive,(uint64_t) blockno,iobuf) == -1) {		/* read block */
	kernelfree(iobuf);
	return(-1);
}
...
while(write_size > 0) {
	...
	if(write_size > (bpb->sectorsperblock*bpb->sectorsize)) {
	...
	/* if writing data that crosses a block boundary, find next block to write data */

	if((write_file_record.currentpos+count+blockoffset) >= round_up(write_file_record.currentpos,(bpb->sectorsperblock*bpb->sectorsize))) {
		if(blockio(DEVICE_WRITE,write_file_record.drive,(uint64_t) blockno,iobuf) == -1) {		/* write block */
			kernelfree(iobuf);
			return(-1);
		}
...
if(fat_is_long_filename(split.filename)) {			/* long filename */
	...
	returnvalue=fat_update_long_filename(_FILE,write_file_record.drive,(uint64_t) write_file_record.findlastblock,write_file_record.dirent,&write_file_record);
...
if(blockio(DEVICE_WRITE,write_file_record.drive,write_file_record.findlastblock,blockbuf) == -1) {	/* write block */
	kernelfree(blockbuf);
	return(-1); 
}
...
if(fattype == 12) {				/* FAT 12 */
	...
	while(rb < (fatsize*bpb->sectorsperblock)) {
		...
		while(entrycount < bpb->sectorsize*2) {
			...
			if((whichentry > 2) && (shortentry == 0)) {
				setlasterror(NO_ERROR);
				return(whichentry);		/* found free entry */
			}
			...
			if((whichentry > 2) && (second_shortentry == 0)) {
				setlasterror(NO_ERROR);
				return(whichentry);		/* found free entry */
			}
...
else if(fattype == 16) {				/* FAT 16 */
	for(count=bpb->reservedsectors;count < bpb->reservedsectors+bpb->sectorsperfat;count++) {  
...
else if(fattype == 32) {		/* FAT 32 */
	for(count=bpb->reservedsectors;count < bpb->reservedsectors+bpb->fat32sectorsperfat;count++) {  
		if(blockio(DEVICE_READ,drive,count,buf) == -1) {			/* read block */ 
			setlasterror(NO_ERROR);
 ...
 *   Until either block=0xff8 (fat 12), 0xfff8 (fat 16), 0xffffffff8 (fat 32) 
 *  	Read block
...
for(c=1;c<tc;c++) {
	/* search until end of directory chain */
	...
	} while((fattype == 12 && rb <= 0xff8) || (fattype == 16 && rb <= 0xfff8) || (fattype == 32 && rb <= 0x0ffffff8));
...
if(fattype == 12) {					/* fat12 */
	entryno=block+(block/2);
	blockno=(uint64_t) bpb->reservedsectors+(entryno/bpb->sectorsize);	/* FAT sector number */
	entry_offset=entryno % bpb->sectorsize;	/* offset into FAT */
	...
	if(block & 1) {					/* block is odd */
		buf[entry_offset]=(block_low_word << 4) | (buf[entry_offset] & 0x0f);
		buf[entry_offset+1]=(block_low_word >> 4);
	}
	else
	{
		buf[entry_offset]=block_low_word & 0xff;
		buf[entry_offset+1]=(block_low_word & 0xf0) | (block_low_word >> 8);
	}
}
	...
	*(uint16_t*)&buf[entry_offset]=block_low_word;		/* update FAT */
}
else if(fattype == 32) {					/* fat32 */
	entryno=block*4;				
	blockno=(uint64_t) bpb->reservedsectors+(entryno/bpb->sectorsize);	/* FAT sector number */
	entry_offset=entryno % (bpb->sectorsperblock*bpb->sectorsize);	/* offset into fat */
...
while((fattype == 12 && blockcount <= 0xff8) || (fattype == 16 && blockcount <= 0xfff8) || (fattype == 32 && blockcount <= 0x0ffffff8)) {	/* until end of chain */
...
while((fattype == 12 && block <= 0xff8) || (fattype == 16 && block <= 0xfff8) || (fattype == 32 && block < 0x0ffffff8)) {	/* until end of chain */
...
do {
	...
	block=b;
} while((fattype == 12 && block != 0xFF8) || (fattype == 16 && block !=0xFFF8) || (fattype == 32 && block !=0xFFFFFF8));
...
for(block_count_loop=block;block_count_loop < (uint64_t)  block+blockcount;block_count_loop++) {
		if(blockio(DEVICE_WRITE,splitbuf.drive,(uint64_t) block_count_loop,blockptr) == -1) {	/* write block for entry */
			kernelfree(blockbuf);
			return(-1);
		}
...
/*
 * Update FAT32 free block count
 *
 * In: drive		Drive
 *	blockcount	Number of blocks to add or subtract
 *
...
blockptr=kernelalloc(bpb->sectorsize*bpb->sectorsperblock);
if(blockptr == NULL) return(-1);

if(blockio(DEVICE_READ,drive,bpb->fat32infosector,blockptr) == -1) {	/* read FSInfo sector */
	kernelfree(blockptr);
	return(-1);
}
...
if(fsinfo.freeblockcount != 0xFFFFFFFF) {
	...
	if((fsinfo.leadsignature != 0x41615252) || (fsinfo.secondsignature != 0x61417272) || (fsinfo.trailsignature != 0xAA5500000)) {
		kernelfree(blockptr);
		return(-1);
	}

...
unsigned int addpage(unsigned int mode, size_t process,size_t page,void *physaddr); 
unsigned int page_init(size_t process);
unsigned int removepage(size_t page,size_t process);
unsigned int freepages(size_t process);
unsigned int loadpagetable(size_t pid);
unsigned int findfreevirtualpage(size_t size,unsigned int alloc,size_t process);
...
unsigned int addpage(uint32_t mode,size_t process,uint32_t page,void *physaddr); 
struct ppt *next;
unsigned int pt;
unsigned int pd;
unsigned int pdp;
...
/* memory map*/
 ...
 while(c < MEMBUF_START+(((MEM_SIZE/PAGE_SIZE+PAGE_SIZE)+PAGE_SIZE)*sizeof(unsigned int))) {
...
unsigned int removepage(uint64_t page,unsigned int process) {
...
uint64_t getphysicaladdress(unsigned int process,uint32_t virtaddr) {
struct ppt *next;
unsigned int pt;
unsigned int pd;
unsigned int pdp;
...
unsigned int addpage_user(uint64_t page,unsigned int process,void *physaddr) { 
 return(addpage_int(PAGE_USER+PAGE_RW+PAGE_PRESENT,process,page,physaddr));
}
unsigned int addpage_system(uint64_t page,unsigned int process,void *physaddr) { 
